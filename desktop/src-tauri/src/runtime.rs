...
pub fn required_java(game: &str) -> Result<u32, String> {
    ...
    match parts.as_slice() {
        ...
        [1, minor, rest @ ..] => Ok(
            if *minor > 20 || (*minor == 20 && rest.first().copied().unwrap_or(0) >= 5) {
                21
            } else if *minor >= 18 {
                17
            } else if *minor == 17 {
...
pub fn java_candidates(major: u32) -> Vec<PathBuf> {
    ...
    for key in [format!("JAVA_{major}_HOME"), "JAVA_HOME".to_owned()] {
        if let Some(path) = std::env::var_os(key) {
            candidates.push(PathBuf::from(path).join("bin/java.exe"));
        }
    }
...
pub fn ready_line(line: &str) -> bool {
...
pub fn public_address(value: &str) -> Result<String, String> {
    ...
    {
        return Err(
            "Enter only a public hostname and optional port, without https:// or a path.".into(),
        );
    }
    ...
    if let Some(port) = parts.next() {
        if port.parse::<u16>().ok().filter(|p| *p > 0).is_none() {
            return Err("Invalid public port.".into());
        }
    }
...
mod tests {
    use super::*;
    #[test]
    fn process_paths_preserve_spaces_and_unc() {
        for (input, expected) in [
            ...
            assert_eq!(
                process_path(std::path::Path::new(input)),
                PathBuf::from(expected)
            );
        }
    }
    #[test]
    fn java_version_boundaries() {
        for (game, major) in [
            ("1.16.5", 8),
            ("1.17.1", 16),
            ("1.18.2", 17),
            ...
            assert_eq!(required_java(game), Ok(major));
        }
        assert!(required_java("latest").is_err());
    }
    #[test]
    fn parses_old_and_new_java() {
        assert_eq!(java_major("java version \"1.8.0_412\""), Some(8));
        assert_eq!(java_major("openjdk version \"21.0.4\""), Some(21));
        assert_eq!(java_major("missing"), None);
    }
    #[test]
    fn readiness_requires_minecraft_marker() {
        assert!(ready_line(
            "[Server thread/INFO]: Done (2.4s)! For help, type help"
        ));
        assert!(!ready_line("Starting Minecraft"));
    }
    #[test]
    fn validates_public_addresses() {
        assert_eq!(
            public_address("example.craft.playit.gg:12345").unwrap(),
            "example.craft.playit.gg:12345"
        );
        for bad in [
            "https://foo.bar",
            "foo.bar:0",
            ...
            assert!(public_address(bad).is_err());
        }
        assert_eq!(public_address("").unwrap(), "");
    }
}
