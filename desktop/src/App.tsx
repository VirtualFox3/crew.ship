...
type ServerAddress = { lanAddress?: string; publicAddress?: string; port: number; playitConfigured?: boolean; tunnelRegion: string };
type PublicAddressProbe = { reachable: boolean; message: string };
...
function App() {
  ...
  const [publicProbe, setPublicProbe] = useState<PublicAddressProbe>();
        ...
        <section className="content">
          ...
          {view === "server" && (activeServer ? <>
            ...
            <section className="connection-focus" aria-label="Server connection address">
              <div className="connection-focus-copy"><span>{addresses[activeServer.id]?.publicAddress ? "YOUR PUBLIC SERVER IP" : "YOUR LAN SERVER IP"}</span><strong>{joinAddress(addresses[activeServer.id]) ?? "CHECKING ADDRESS…"}</strong><small>{addresses[activeServer.id]?.publicAddress ? "Saved Playit address. Test it below before sharing it with friends." : "LAN only. Configure Playit below to share with internet players."}{!ready[activeServer.id] && " Server is not ready for players yet."}</small></div>
            ...
            <div className="server-workspace">
              ...
              <section className="server-detail-panel">
                {serverTab === "server" && <div className="detail-overview"><span className={`status-dot ${ready[activeServer.id] ? "online" : ""}`} /><h2>{ready[activeServer.id] ? "Online" : running[activeServer.id] ? "Starting / stopping" : "Offline"}</h2><p>{ready[activeServer.id] ? "Minecraft is ready for players." : running[activeServer.id] ? "Minecraft has not confirmed readiness. Check Console for progress." : "Start this server to accept players."}</p><div className="detail-actions"><button className="ghost" onClick={() => void openServer(activeServer, "console")}>OPEN CONSOLE</button><button className="ghost" onClick={() => void openServer(activeServer, "options")}>SERVER OPTIONS</button><button className="ghost" onClick={() => void openServer(activeServer, "access")}>SHARE ACCESS</button></div><form key={activeServer.id} className="public-address-form" onSubmit={async (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const address = String(data.get("publicAddress") ?? ""); const region = String(data.get("region") ?? "global"); try { await invoke("set_tunnel_region", { id: activeServer.id, region }); await invoke("save_public_address", { id: activeServer.id, address }); setPublicProbe(undefined); await refreshStatuses(); setNotice("Tunnel region and public address saved. Restart the server to apply the region to a Crew.Ship-created tunnel."); } catch (cause) { setError(errorMessage(cause)); } }}><label htmlFor="public-address">PUBLIC SERVER ADDRESS</label><p>{addresses[activeServer.id]?.playitConfigured ? <>In Playit, make a <b>Minecraft Java</b> tunnel to <b>127.0.0.1:{addresses[activeServer.id]?.port ?? "…"}</b>. Copy its full address and port here, then test it.</> : <>Connect Playit in Ship settings to enable public hosting. Local port: <b>{addresses[activeServer.id]?.port ?? "…"}</b>.</>}</p><input id="public-address" name="publicAddress" defaultValue={addresses[activeServer.id]?.publicAddress ?? ""} placeholder="fnaf.example.playit.gg:12345" /><label className="property-field"><span>PLAYIT REGION</span><select name="region" defaultValue={addresses[activeServer.id]?.tunnelRegion ?? "global"}><option value="global">Automatic / Global</option><option value="chicago-illinois">Chicago, Illinois</option><option value="dallas-texas">Dallas, Texas</option><option value="denver-colorado">Denver, Colorado</option><option value="new-york">New York</option><option value="los-angeles-california">Los Angeles, California</option><option value="north-america">North America</option><option value="europe">Europe</option><option value="asia">Asia</option><option value="australia">Australia</option></select></label><small>Choose the region nearest this computer. Existing tunnels must be moved in Playit; Crew.Ship uses this choice for a newly created automatic tunnel.</small><div className="detail-actions"><button className="primary" type="submit" disabled={!addresses[activeServer.id]?.playitConfigured}>SAVE TUNNEL SETTINGS</button>{addresses[activeServer.id]?.publicAddress && <button className="ghost" type="button" disabled={busy === "public-test"} onClick={async () => { setBusy("public-test"); setError(undefined); try { setPublicProbe(await invoke<PublicAddressProbe>("test_public_address", { id: activeServer.id })); } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(undefined); } }}>{busy === "public-test" ? "TESTING…" : "TEST PUBLIC ADDRESS"}</button>}{addresses[activeServer.id]?.playitConfigured && <button className="ghost" type="button" onClick={() => void openUrl("https://playit.gg/account/tunnels")}>OPEN PLAYIT TUNNELS ↗</button>}</div>{publicProbe && <p className={`tunnel-feedback ${publicProbe.reachable ? "success" : "error"}`}>{publicProbe.reachable ? "Reachable: true. " : "Reachable: false. "}{publicProbe.message}</p>}</form></div>}
